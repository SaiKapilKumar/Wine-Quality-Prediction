...
def main():
    ...
    with st.expander('Data Description'):
        ...
        option = st.selectbox('',['Data','Describe-Data','Info', 'Number of Good and Bad Wine', 'Original Data'])
        if option == 'Data':
            st.write(data)
    ...
    # Sliders for input
    with st.expander('Select Attributes by sliders'):
    ...
    # Text box for input
    with st.expander('Select Attributes by text input', expanded=True):
        # use text box for input all attributes at once
        user_input_text = st.text_area('Enter all attributes separated by comma', '10.42, 8.32, 0.53, 0.27, 2.54, 0.087, 16, 46, 0.997, 3.31, 0.66')
        user_input_text = list(map(float, user_input_text.replace(' ', '').split(',')))

    st.markdown('</br>', unsafe_allow_html=True)
    # Logistic Regression
    if model_option == 'Logistic Regression':
        modelLR, X_train, X_test, y_train, y_test, y_pred = logistic_regression(data)
        st.sidebar.write('Training Score :', modelLR.score(X_train, y_train))
        st.sidebar.write('Testing Score :', modelLR.score(X_test, y_test))
        ...
        if st.button('Predict Wine Quality', key=1):
            ...
            prediction = modelLR.predict([user_input])
            if prediction == [0]:
        ...
        if st.button('Predict Wine Quality (Text Input)', key=11):
            prediction = modelLR.predict([user_input_text])
            if prediction == [0]:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:red">bad<b></p>', unsafe_allow_html=True)
            else:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:green">good<b></p>', unsafe_allow_html=True)
        ...
        report = st.selectbox('Select Report', ['Classification Report', 'Confusion Matrix'], key=111)
    ...
    # SGD Classifier
        ...
        modelSGD, X_train, X_test, y_train, y_test, y_pred = SGD_classifier(data)
        st.sidebar.write('Training Score :', modelSGD.score(X_train, y_train))
        st.sidebar.write('Testing Score :', modelSGD.score(X_test, y_test))

        if st.button('Predict Wine Quality', key=2):
            user_input = [alcohol, fixed_acidity, volatile_acidity, citric_acid, residual_sugar, chlorides, free_sulfur_dioxide, total_sulfur_dioxide, density, pH, sulphates]
            prediction = modelSGD.predict([user_input])
            if prediction == [0]:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:red">bad<b></p>', unsafe_allow_html=True)
            else:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:green">good<b></p>', unsafe_allow_html=True)

        if st.button('Predict Wine Quality (Text Input)', key=22):
            prediction = modelSGD.predict([user_input_text])
            if prediction == [0]:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:red">bad<b></p>', unsafe_allow_html=True)
            else:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:green">good<b></p>', unsafe_allow_html=True)
        time.sleep(2)
        # generate a report as dropdown
        report = st.selectbox('Select Report', ['Classification Report', 'Confusion Matrix'], key=222)
        if report == 'Classification Report':
    ...
    # Decision Tree
        ...
        if st.button('Predict Wine Quality', key=3):
            user_input = [alcohol, fixed_acidity, volatile_acidity, citric_acid, residual_sugar, chlorides, free_sulfur_dioxide, total_sulfur_dioxide, density, pH, sulphates]
        ...
        if st.button('Predict Wine Quality (Text Input)', key=33):
            prediction = model.predict([user_input_text])
            if prediction == [0]:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:red">bad<b></p>', unsafe_allow_html=True)
            else:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:green">good<b></p>', unsafe_allow_html=True)
        time.sleep(2)
        # generate a report as dropdown
        report = st.selectbox('Select Report', ['Classification Report', 'Confusion Matrix'], key=333)
        if report == 'Classification Report':
    ...
    # Random Forest
        ...
        if st.button('Predict Wine Quality (Text Input)', key=44):
            prediction = model.predict([user_input_text])
            if prediction == [0]:
                st.write('<p style="font-size:40px;">The wine quality is <b style="color:red">bad<b></p>', unsafe_allow_html=True)
            else:
        ...
        time.sleep(2)
        # generate a report as dropdown
        report = st.selectbox('Select Report', ['Classification Report', 'Confusion Matrix'], key=444)
        if report == 'Classification Report':
    ...
    st.markdown(
        "More infos and :star: at [github.com/SaiKapilKumar/Wine-Quality-Prediction](https://github.com/SaiKapilKumar/Wine-Quality-Prediction)]"
    )
    ...
    with st.sidebar.expander('About'):
        st.write('''
        This is a web app to predict the quality of wine. The data is taken from [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/datasets/wine+quality). The data contains 1599 rows and 12 columns. The columns are as follows:
        - fixed acidity
        - volatile acidity
        - citric acid
        - residual sugar
